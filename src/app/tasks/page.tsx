  ...
  return (
    <div className="relative min-h-screen min-h-dvh w-full flex flex-col items-center text-white overflow-hidden bg-[var(--background)] font-sans">
      ...
      <main className="relative z-10 w-full max-w-7xl px-6 sm:px-10 py-10 sm:py-16 flex flex-col gap-10">
        ...
        {/* AI Day Plan */}
        <section className="rounded-3xl bg-white/95 dark:bg-gray-900/50 border border-white/50 dark:border-gray-800/40 backdrop-blur-xl p-5">
            ...
            <ol className="space-y-2 text-sm">
              {plan.map((b) => (
                <li key={b.id} className="flex items-center justify-between">
                  ...
                  <span className="opacity-70 ml-2">({b.estimateMin}m)</span>
                </li>
              ))}
            </ol>
          ) : (
            <div className="text-sm opacity-70">No plan yet — click “Plan my day”.</div>
          )}
          {!!nudges.length && (
            <ul className="mt-3 text-xs opacity-80 list-disc pl-4 space-y-1">
